...
const fetchStatus = async () => { try { const res = await fetch(${API_BASE_URL}/status); const data = await res.json(); setStatus(data.status || 'Unknown'); } catch (err) { setStatus('Error'); } };
const fetchLogs = async () => { try { const res = await fetch(${API_BASE_URL}/logs); const data = await res.json(); setLogs(data.logs || []); } catch (err) { setLogs(['Error fetching logs']); } };
const fetchBrokers = async () => { try { const res = await fetch(${API_BASE_URL}/brokers); const data = await res.json(); setBrokers(data.brokers || []); } catch (err) { setBrokers(['Error fetching brokers']); } };
const startBot = async () => { try { await fetch(${API_BASE_URL}/start-bot, { method: 'POST' }); fetchStatus(); } catch (err) { alert('Error starting bot'); } };
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="w-full h-40 bg-gray-800 rounded">[Chart Placeholder]</div>
      </CardContent>
    </Card>
  </div>
